...
def button_2_clicked():
    admin_id = entry_1.get()
    admin_password = entry_2.get()
    ...
    # Fetch EMPLOYEE_ID for the given ADMIN_ID
    query_employee_id = "SELECT EMPLOYEE_ID FROM ADMIN WHERE ADMIN_ID = %s"
    cursor.execute(query_employee_id, (admin_id,))
    ...
    if employee_id_result:
        ...
        if password_result and password_result[0] == admin_password:
            # Valid credentials - Admin authenticated
        ...
        # Admin ID not found - Show a popup message
        messagebox.showerror("Admin ID not found", "Invalid Admin ID")
