...
def button_2_clicked():
    ...
    # Fetch EMPLOYEE_ID for the given DOCTOR_ID
    query_employee_id = "SELECT EMPLOYEE_ID FROM DOCTOR WHERE DOCTOR_ID = %s"
    cursor.execute(query_employee_id, (doctor_id,))
    ...
    if employee_id_result:
        ...
        if password_result and password_result[0] == doctor_password:
            # Valid credentials - Doctor authenticated
            # You can open the next window or perform actions here
            messagebox.showinfo("Login Successful", "Doctor Logged in successfully")
        ...
        # Doctor ID not found - Show a popup message
        messagebox.showerror("Doctor ID not found", "Invalid Doctor ID")
