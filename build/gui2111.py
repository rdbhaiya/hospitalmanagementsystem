...
def button_2_clicked():
    Pharmacist_id = entry_1.get()
    Pharmacist_password = entry_2.get()
    ...
    # Fetch EMPLOYEE_ID for the given Pharmacist_id
    query_employee_id = "SELECT EMPLOYEE_ID FROM PHARMACIST  WHERE PHARMACIST_ID = %s"
    cursor.execute(query_employee_id, (Pharmacist_id,))
    ...
    if employee_id_result:
        ...
        if password_result and password_result[0] == Pharmacist_password:
            # Valid credentials - Pharmacist authenticated
            # You can open the next window or perform actions here
            messagebox.showinfo("Login Successful", "Pharmacist Logged in successfully")
        ...
        # Pharmacist ID not found - Show a popup message
        messagebox.showerror("Pharmacist ID not found", "Invalid Pharmacist ID")
